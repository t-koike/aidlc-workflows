...
**Worktree mapping.** Create: `{{INVOKE}} __delegate worktree create --slug <bolt-slug> --base main`. Merge: `--target main --strategy squash`. Each Bolt = one squash commit on `main`.
...
3. **Create**: invoke `{{INVOKE}} __delegate worktree create --slug <bolt-slug> --base main`.
4. **Merge** (after Bolt gate approval): caller must be on `main` at the main checkout. Invoke `{{INVOKE}} __delegate worktree merge --slug <bolt-slug> --target main --strategy squash --message "<commit message>"`.
...
3. **Create**: `{{INVOKE}} __delegate worktree create --slug <bolt-slug> --base main`.
4. **Merge**: `{{INVOKE}} __delegate worktree merge --slug <bolt-slug> --target main --strategy <squash|merge> [--message "<msg>"]`. With `--strategy merge`, a no-fast-forward merge commit preserves the bolt branch's individual commits.
...
4. **Create**: `{{INVOKE}} __delegate worktree create --slug <bolt-slug> --base <integration>`.
5. **Merge**: caller must be on `<integration>` at the main checkout. `{{INVOKE}} __delegate worktree merge --slug <bolt-slug> --target <integration> --strategy <merge|squash>`.
...
3. **Create**: `{{INVOKE}} __delegate worktree create --slug <bolt-slug> --base <line>`.
4. **Merge**: caller on `<line>` at the main checkout. `{{INVOKE}} __delegate worktree merge --slug <bolt-slug> --target <line> --strategy merge`.
