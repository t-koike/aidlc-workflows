...
```
Loop:
  1. directive = `aidlc __delegate orchestrate next <the user's text AFTER the $aidlc marker, verbatim>`
...
Source of truth: one file per scope under `.codex/scopes/aidlc-<name>.md` (identity + keywords + description + skeleton default) plus each stage's `scopes:` frontmatter (membership), transposed into the compiled grid at `aidlc __delegate graph compile`; regenerate this table with `aidlc __delegate utility scope-table`.
