...
- `dist-release/<harness>/` — **GENERATED, committed, drift-guarded binary channel.** The same authored content with harness-local Bun invocations projected through the native `aidlc` dispatcher. `scripts/package-release.ts` packages these trees with target binaries, checksums, `version.json`, and `install.sh`.
