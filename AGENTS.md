...
- `dist-release/<harness>/` — **GENERATED, committed, drift-guarded native release staging.** Projected separately from the Bun-invoked `dist/<harness>/` copy channel so hooks, generated commands, adapters, and trust entries invoke the compiled `aidlc` dispatcher. `scripts/package-release.ts` packages these trees with target binaries, checksums, `version.json`, and both installers.
...
- **Tools** (`tools/`) — TypeScript CLI sources, all prefixed `aidlc-*.ts`; the copy channel invokes them through Bun and the native release channel routes them through the compiled `aidlc` dispatcher
- **Hooks** (`hooks/`) — 13 framework hooks, all prefixed `aidlc-*.ts`, covering audit emission, sensor dispatch, runtime-graph compile, session lifecycle, state validation, subagent tracking, statusline rendering, human-presence mint on prompt submit, forwarding-loop enforcement (the `Stop` hook — the first flow-altering hook), and reviewer read-scope enforcement (the PreToolUse reviewer-scope hook — the second)
