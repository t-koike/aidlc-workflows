...
- `dist-release/<harness>/` — **GENERATED, committed, drift-guarded release staging.** Byte-identical to the native `dist/<harness>/` projection. `scripts/package-release.ts` packages these trees with target binaries, checksums, `version.json`, and both installers.
...
- **Tools** (`tools/`) — TypeScript CLI sources, all prefixed `aidlc-*.ts`; shipped host wiring routes them through the compiled `aidlc` dispatcher
