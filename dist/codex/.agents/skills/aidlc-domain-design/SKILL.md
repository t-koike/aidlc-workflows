...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts next --stage domain-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts report --single --stage domain-design --result completed
   ```
