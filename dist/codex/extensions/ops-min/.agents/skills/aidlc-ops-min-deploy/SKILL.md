...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts next --stage ops-min-deploy --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts report --single --stage ops-min-deploy --result completed
   ```
