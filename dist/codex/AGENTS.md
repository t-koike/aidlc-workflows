...
- **Runtime**: Framework commands run through `bun .codex/tools/aidlc.ts`; keep that command and its runtime available.
...
- **Hook runtime**: Framework hooks run through `bun .codex/tools/aidlc.ts`; keep that command and its runtime available.
- **Permissions**: `.codex/rules/default.rules` (Starlark prefix rules) pre-allows the projected framework runtime and `git worktree`/`commit`/`add`, so workflows run without per-call prompts. The sandbox is `workspace-write`; commands outside the allowlist prompt.
