...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .aidlc/tools/aidlc-orchestrate.ts next --stage reverse-engineering --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .aidlc/tools/aidlc-orchestrate.ts report --single --stage reverse-engineering --result completed
   ```
