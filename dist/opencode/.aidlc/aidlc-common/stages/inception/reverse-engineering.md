...
`bun .aidlc/tools/aidlc.ts __delegate orchestrate report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .aidlc/tools/aidlc.ts __delegate utility codekb-path --repo <repo>
```
...
`bun .aidlc/tools/aidlc.ts __delegate orchestrate report --stage reverse-engineering --result <outcome>`.
