...
```bash
bun .aidlc/tools/aidlc.ts __delegate state practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .aidlc/tools/aidlc.ts __delegate orchestrate report --stage
   practices-discovery --result awaiting-approval`.
2. Log the affirmation question with `bun .aidlc/tools/aidlc.ts __delegate log decision`.
...
5. Log the exact answer with `bun .aidlc/tools/aidlc.ts __delegate log answer`.
