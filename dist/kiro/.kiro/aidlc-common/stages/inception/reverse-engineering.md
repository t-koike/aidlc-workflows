...
`bun .kiro/tools/aidlc.ts __delegate orchestrate report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .kiro/tools/aidlc.ts __delegate utility codekb-path --repo <repo>
```
...
`bun .kiro/tools/aidlc.ts __delegate orchestrate report --stage reverse-engineering --result <outcome>`.
