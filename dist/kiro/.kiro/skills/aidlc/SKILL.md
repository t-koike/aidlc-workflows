...
- **State sync is conductor-owned here.** This harness has no task-list hook: after each stage transition the native `state` routes do all bookkeeping (`advance`, `approve`, etc. — same as everywhere), and the stage-protocol's TaskUpdate steps are satisfied by Kiro's `todo_list` tool when available — keep a task list for visibility, but know that state-file sync rides on those routes, not on a todo hook.
