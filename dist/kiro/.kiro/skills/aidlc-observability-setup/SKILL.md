...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   aidlc __delegate orchestrate next --stage observability-setup --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   aidlc __delegate orchestrate report --single --stage observability-setup --result completed
   ```
