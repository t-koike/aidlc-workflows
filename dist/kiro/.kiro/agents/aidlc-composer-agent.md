...
1. **Detect.** Run `bun .kiro/tools/aidlc.ts __delegate utility detect --json`.
...
6. **Write (front/report, after approval).** Author BOTH files - a
   ...
   **NEVER run `bun .kiro/tools/aidlc.ts __delegate graph compile` after the write.** Compile is the
