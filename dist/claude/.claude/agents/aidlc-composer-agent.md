...
1. **Detect.** Run `bun .claude/tools/aidlc.ts __delegate utility detect --json`.
...
4. **Validate.** Before the proposal is shown, write the proposed grid to a
   ...
   `bun .claude/tools/aidlc.ts __delegate graph validate-grid --proposal <path> --project-type <greenfield|brownfield>`
...
6. **Write (front/report, after approval).** Author BOTH files - a
   ...
   **NEVER run `bun .claude/tools/aidlc.ts __delegate graph compile` after the write.** Compile is the
...
- Making a scope inferable is an explicit human choice at the gate ("make
  ...
  `bun .claude/tools/aidlc.ts __delegate graph validate-grid --proposal <path> --keywords <granted,csv>`
