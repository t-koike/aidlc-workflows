...
1. [ ] **Use the engine for every lifecycle transition** — before the prompt, `bun .claude/tools/aidlc.ts __delegate orchestrate report --stage <slug> --result awaiting-approval`; after the response, report `approved` or `rejected`; after revision work, report `revised`. When the active stage's own condition proves it does not apply, report `skipped --reason "<reason>"`. Never call lifecycle verbs on `bun .claude/tools/aidlc.ts __delegate state` directly. The engine emits the correct audit events and routes only on approval, completion, or a justified skip. Do NOT call `bun .claude/tools/aidlc.ts __delegate audit append` separately. (§2)
2. [ ] **Log questions via `bun .claude/tools/aidlc.ts __delegate log`** — before presenting a structured question: `bun .claude/tools/aidlc.ts __delegate log decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .claude/tools/aidlc.ts __delegate log answer --stage <slug> --details "<exact choice>"`. (§3)
...
2. After the learnings answer is logged: `bun .claude/tools/aidlc.ts __delegate orchestrate report --stage <slug> --result awaiting-approval` — the engine marks `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate.
...
1. Report the outcome through `bun .claude/tools/aidlc.ts __delegate orchestrate report`; the engine selects and runs the atomic state transition.
...
1. BEFORE presenting the approval question: `bun .claude/tools/aidlc.ts __delegate orchestrate report --stage <slug> --result awaiting-approval`.
2. AFTER user response: report `approved --user-input "<choice>"` or `rejected --user-input "<feedback>"`. After revision work, report `revised` before re-presenting. Never call lifecycle verbs on `bun .claude/tools/aidlc.ts __delegate state` directly.
...
**`[S]` behavior:**
- Set by the Stage/Phase Jump handler (`bun .claude/tools/aidlc.ts __delegate jump execute`) for in-scope stages before the jump target, or by `bun .claude/tools/aidlc.ts __delegate orchestrate report --result skipped` when the active stage's own applicability check justifies a skip
...
# Stage completion is reported through bun .claude/tools/aidlc.ts __delegate orchestrate; no manual checkbox write.
```

**Field updates** (aidlc-state.md) are owned by dedicated tool commands. Generic
`bun .claude/tools/aidlc.ts __delegate state set` and lifecycle verbs are engine-internal; stage prose must
use `bun .claude/tools/aidlc.ts __delegate orchestrate report`, `bun .claude/tools/aidlc.ts __delegate utility scope-change` /
...
**Stage advancement** is engine-internal. `bun .claude/tools/aidlc.ts __delegate orchestrate report` selects `advance`, `approve`, `finalize`, or `complete-workflow` and invokes it with an ownership marker. Conductors never invoke those `bun .claude/tools/aidlc.ts __delegate state` lifecycle verbs directly.
