...
3. **Recompile.** Run `aidlc __delegate graph compile` to transpose the tags into `scope-grid.json`, then refresh SKILL.md's compiled scope-table from `aidlc __delegate utility scope-table`. Run `aidlc __delegate graph compile --check` and `aidlc __delegate utility scope-table --check` to confirm no drift (exit 0).
