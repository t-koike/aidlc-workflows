...
- **Adding or renaming an artifact?** Read [Artifact Vocabulary](16-artifact-vocabulary.md) — the chapter explains the naming rules, the stability policy (rename/removal = major, addition = minor), and points at `aidlc graph artifacts` for the live list. The registry is derived from stage files, not written.
