...
The engine is authored at `core/tools/aidlc-orchestrate.ts` and projected into each harness as `<harness-dir>/tools/aidlc-orchestrate.ts` (e.g. `.claude/tools/`). Installed harnesses execute it in-process through the native `aidlc __delegate orchestrate` route, with exactly three subcommands: `next`, `report`, and `park`; users do not need Bun or invoke the projected TypeScript file directly.
...
|--------|----------------|--------------------------|
| `print` | Yes | Do exactly what `directive.message` says — it is authoritative. Two shapes: **terminal** (names a read-only utility such as status/help/doctor/version; run it, print stdout verbatim, STOP) and **run-then-continue** (names a mutating route such as a scope change, a jump `execute`, or the workflow-birth `intent-birth --scope <scope>` emitted when the user explicitly names a scope — flag or positional — on a fresh workspace; run it, then return to step 1 of the loop). The mutation lives in the named route, never in `next`. |
